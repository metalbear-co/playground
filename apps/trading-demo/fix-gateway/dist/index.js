...
function symbolToProductId(symbol) {
...
async function maybeCreateShopOrder(fields) {
    if (!orderServiceUrl)
        return null;
...
async function handleFixMessage(raw, peer) {
    ...
    console.log("[FixGateway] FIX from %s type=%s clOrdId=%s symbol=%s raw=%s", peer, msgType, clOrdId, fields["55"] || "-", trimmed);
    let orderId = null;
    ...
    const server = net.createServer((socket) => {
        ...
        socket.on("data", (chunk) => {
            ...
            while ((idx = buffer.indexOf("|10=")) !== -1) {
                ...
                void handleFixMessage(message, peer).catch((err) => console.error("[FixGateway] handle error:", err));
            }
        });
        socket.on("end", () => {
            if (buffer.trim()) {
                void handleFixMessage(buffer, peer).catch((err) => console.error("[FixGateway] handle error:", err));
                buffer = "";
            }
        });
    });
