...
function ProductTile({
  ...
  if (variant === "featured") {
    return (
      <button
        type="button"
        onClick={onClick}
        className={`group relative flex h-full min-h-[280px] w-full cursor-pointer flex-col overflow-hidden rounded-2xl border border-slate-300 bg-slate-100 text-left shadow-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-[#6a4ff5]/15 hover:border-[#6a4ff5]/30 animate-card-reveal ${elevatedClass}`}
  ...
  if (variant === "wide") {
    return (
      <button
        type="button"
        onClick={onClick}
        className={`group relative flex w-full cursor-pointer flex-col overflow-hidden rounded-2xl border border-slate-300 bg-white text-left shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-[#6a4ff5]/30 hover:shadow-xl hover:shadow-[#6a4ff5]/10 animate-card-reveal sm:flex-row ${elevatedClass}`}
        style={{ animationDelay: `${delay}s` }}
      >
        {product.is_new && <NewBadge size="default" />}
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`group relative flex w-full cursor-pointer flex-col overflow-hidden rounded-2xl border border-slate-300 bg-white text-left shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-[#6a4ff5]/30 hover:shadow-xl hover:shadow-[#6a4ff5]/10 animate-card-reveal ${elevatedClass}`}
      style={{ animationDelay: `${delay}s` }}
    >
      {product.is_new && <NewBadge size="default" />}
              ...
              <>
                ...
                >
                  ...
                  {featured && (
                    ...
                    >
                      <ProductTile product={featured} variant="featured" delay={0} elevated onClick={() => setSelectedProduct(featured)} />
                    </div>
                  )}
                  ...
                  {rest[0] && (
                    ...
                    >
                      <ProductTile product={rest[0]} variant="standard" delay={0.06} elevated onClick={() => setSelectedProduct(rest[0])} />
                    </div>
                  )}
                  {rest[1] && (
                    <div className="md:col-span-2 md:row-span-1">
                      <ProductTile product={rest[1]} variant="standard" delay={0.12} elevated onClick={() => setSelectedProduct(rest[1])} />
                    </div>
                  )}
                  {/* Bottom row - product 4 as wide horizontal tile */}
                  {rest[2] && (
                    <div className="md:col-span-4">
                      <ProductTile product={rest[2]} variant="wide" delay={0.18} elevated onClick={() => setSelectedProduct(rest[2])} />
                    </div>
                  )}
