  ...
  if (variant === "wide") {
    return (
      <button
        type="button"
        onClick={() => onSelect(product)}
        className={`group relative flex w-full flex-col overflow-hidden rounded-2xl border border-slate-300 bg-white shadow-sm transition-all duration-300 hover:-translate-y-1 hover:border-[#6a4ff5]/30 hover:shadow-xl hover:shadow-[#6a4ff5]/10 animate-card-reveal sm:flex-row text-left ${elevatedClass}`}
        style={{ animationDelay: `${delay}s` }}
      >
        {product.is_new && <NewBadge size="default" />}
...
export default function Home() {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-white">
              ...
              <>
                ...
                >
                  ...
                  {featured && (
                    ...
                    >
                      <ProductTile product={featured} variant="featured" delay={0} elevated onSelect={handleSelect} />
                    </div>
                  )}
                  ...
                  {rest[0] && (
                    ...
                    >
                      <ProductTile product={rest[0]} variant="standard" delay={0.06} elevated onSelect={handleSelect} />
                    </div>
                  )}
                  {rest[1] && (
                    <div className="md:col-span-2 md:row-span-1">
                      <ProductTile product={rest[1]} variant="standard" delay={0.12} elevated onSelect={handleSelect} />
                    </div>
                  )}
                  {/* Bottom row - product 4 as wide horizontal tile */}
                  {rest[2] && (
                    <div className="md:col-span-4">
                      <ProductTile product={rest[2]} variant="wide" delay={0.18} elevated onSelect={handleSelect} />
                    </div>
                  )}
      ...
      {selectedProduct && (
        <ProductDialog product={selectedProduct} onClose={handleClose} />
      )}
    </div>
  );
}
