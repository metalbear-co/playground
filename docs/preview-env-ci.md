...
1. **On PR open / push:** Workflow builds frontend and counter images, pushes them with tags `preview-pr-<number>-<sha>`, runs `mirrord preview start` twice (one per deployment) with the same key `pr-<number>`, and posts or updates a single PR comment with the preview URL and header (`baggage: mirrord=pr-<number>`).
2. **Reviewers:** Open the shared link and set the header `baggage: mirrord=pr-<number>` (e.g. via the [mirrord Browser Extension](https://metalbear.com/mirrord/docs/using-mirrord/browser-extension)). The UI shows "Preview: pr-<number>" when the request hits the preview pods.
