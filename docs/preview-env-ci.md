...
2. **Reviewers:** Open the shared link and set the header `X-PG-Tenant: pr-<number>` (e.g. via the [mirrord Browser Extension](https://metalbear.com/mirrord/docs/using-mirrord/browser-extension)). The UI shows "Preview: pr-<number>" when the request hits the preview pods.
