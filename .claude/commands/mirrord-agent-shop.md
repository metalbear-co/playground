...
#### 3a.0 — Preflight: stop any alive preview with the same key

**You MUST do this before every `start` dispatch.** mirrord will not cleanly overlay a second `start` on top of an already-alive preview with the same `preview_key` — the new pods may fail to attach or traffic may route inconsistently. See https://metalbear.com/mirrord/docs/use-cases/preview-environments — previews must be stopped by key before being re-created.

Detect whether a preview for `$BRANCH` is currently alive by looking at the most recent *completed* `start`/`stop` dispatch for this `preview_key`. The workflow's `run-name` is `Preview Shop - <action> <preview_key> caller=<id>`, so the key appears in `displayTitle`.

```bash
# Find the most recent completed start/stop dispatch for this preview key.
LAST_RUN_JSON=$(gh run list \
  ...
  --jq "[.[] | select(.status == \"completed\") | select(.displayTitle | test(\"(start|stop) $BRANCH( |$)\"))] | sort_by(.createdAt) | last")

LAST_ACTION=$(echo "$LAST_RUN_JSON" | jq -r 'if . == null then "none" else (.displayTitle | capture("(?<act>start|stop) ") | .act) end')
LAST_CONCL=$(echo "$LAST_RUN_JSON"  | jq -r 'if . == null then "none" else (.conclusion // "none") end')
echo "Preview history for $BRANCH: last_action=$LAST_ACTION last_conclusion=$LAST_CONCL"

# Preview is considered alive only if the most recent completed action was a successful `start`.
if [ "$LAST_ACTION" = "start" ] && [ "$LAST_CONCL" = "success" ]; then
  echo "Preview '$BRANCH' is alive — stopping before rebuild."
  STOP_CALLER_ID="claude-stop-$(date +%s)"
  gh workflow run preview-shop-pr.yml \
    ...
    -f pr_url="$PR_URL" \
    -f caller_run_id="$STOP_CALLER_ID"

  # Resolve the dispatched stop run
  STOP_RUN_ID=""
  for i in $(seq 1 12); do
    STOP_RUN_ID=$(gh run list \
      --repo metalbear-co/playground \
      --workflow=preview-shop-pr.yml \
      --event workflow_dispatch \
      --limit 20 \
      --json databaseId,displayTitle \
      --jq ".[] | select(.displayTitle | contains(\"caller=$STOP_CALLER_ID\")) | .databaseId" | head -n1)
    [ -n "$STOP_RUN_ID" ] && break
    sleep 5
  done

  # Poll stop — stops are usually quick. Cap at 10 ticks × 30s = 5 min.
  for tick in $(seq 1 10); do
    OUT=$(gh run view "$STOP_RUN_ID" --repo metalbear-co/playground --json status,conclusion --jq '.status + "|" + (.conclusion // "")')
    STATUS=$(echo "$OUT" | cut -d'|' -f1)
    CONCL=$(echo "$OUT" | cut -d'|' -f2)
    echo "preview stop: $STATUS (run $STOP_RUN_ID, tick $tick/10) conclusion=$CONCL"
    [ "$STATUS" = "completed" ] && break
    sleep 30
  done
  # Tolerate a failed stop (nothing to stop / preview already expired) — proceed to start.
else
  echo "No alive preview for $BRANCH (last_action=$LAST_ACTION, conclusion=$LAST_CONCL). Skipping stop."
fi
```

Report the preflight result to the developer in one line before the start dispatch: either `Preview <BRANCH> was alive — stopped (run <id>) before rebuild` or `No alive preview for <BRANCH> — dispatching start`.
...
**Fallback when `gh` is unavailable** (e.g. web Claude Code): dispatch via the REST API. The same preflight-stop logic applies — first `GET /repos/metalbear-co/playground/actions/runs?event=workflow_dispatch&per_page=50` and filter `display_title` for `(start|stop) <BRANCH>(space or end)`; take the most recent `status=completed` entry. If its action is `start` and `conclusion=success`, first POST a `stop` dispatch and poll until it completes before POSTing the `start`.

```
POST https://api.github.com/repos/metalbear-co/playground/actions/workflows/preview-shop-pr.yml/dispatches
Headers: Authorization: Bearer <GITHUB_TOKEN>, Accept: application/vnd.github+json
Body (stop, if preview is alive):
  {"ref": "main", "inputs": {"action":"stop", "preview_key":"<BRANCH>", "pr_number":"<PR_NUMBER>", "pr_url":"<PR_URL>", "caller_run_id":"<STOP_CALLER_ID>"}}
Body (start):
  {"ref": "main", "inputs": {"action":"start", "branch":"<BRANCH>", "base_ref":"main", "preview_key":"<BRANCH>", "pr_number":"<PR_NUMBER>", "pr_url":"<PR_URL>", "caller_run_id":"<CALLER_ID>"}}
```
...
| Detect alive preview | `gh run list --repo metalbear-co/playground --workflow=preview-shop-pr.yml --event workflow_dispatch --limit 50 --json databaseId,displayTitle,status,conclusion,createdAt --jq "[.[] \| select(.status == \"completed\") \| select(.displayTitle \| test(\"(start\|stop) $BRANCH( \|$)\"))] \| sort_by(.createdAt) \| last"` — alive if last action is `start` with conclusion `success` |
...
- **Never skip Phase 3a.0 (preflight stop).** Every `start` dispatch must be preceded by the alive-preview check; if one is found, stop it and wait before starting. Skipping this is the #1 cause of "the preview didn't pick up my changes" flakiness.
- **Never poll the preview run faster than every 60 seconds** (start). Stop polls may use 30s (stops are fast).
